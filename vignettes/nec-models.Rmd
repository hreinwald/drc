...
plot(nec3.fit, type = "none", log = "",
     main = "Comparison of NEC Models",
     xlab = "Ferulic acid concentration (mM)",
     ylab = "Root length (cm)")
points(ryegrass$conc, ryegrass$rootl, pch = 16)
lines(nec2.fit, col = "blue", lwd = 2)
lines(nec3.fit, col = "red", lwd = 2)
lines(nec4.fit, col = "green", lwd = 2)
legend("topright", legend = c("NEC.2", "NEC.3", "NEC.4"),
...
# Fit multiple models
m2 <- drm(response ~ dose, data = mydata, fct = NEC.2())
m3 <- drm(response ~ dose, data = mydata, fct = NEC.3())
m4 <- drm(response ~ dose, data = mydata, fct = NEC.4())

# Compare using model selection
mselect(m2, m3, m4, list = TRUE)
```
